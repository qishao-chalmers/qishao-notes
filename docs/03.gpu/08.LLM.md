...
- traditional llm serving framework usually allocates GPU memory for maximum tokens
  actually the lenght of generated tokens are diverous according to different prompts
- allocation of kv cache continuously for each sequence introducing memory fragmentation
- beam searching, prefix caching, actually different decoding could share the same KV cache
