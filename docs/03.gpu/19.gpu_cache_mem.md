...
1. Data that is accessed on the GPU but does not cause a page fault because the page is present will not upgrade its location in the LRU list. 2. VABlocks that are fully resident on the GPU will never be upgraded in the LRU list until they are evicted and re-faulted.
