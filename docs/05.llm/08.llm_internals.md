...
1. [329 2019] Transformers Without Tears - Improving the Normalization of Self-Attention
2. [1800 2020] On Layer Normalization in the Transformer Architecture
...
1. [329 2019] Transformers Without Tears - Improving the Normalization of Self-Attention
2. [1800 2020] On Layer Normalization in the Transformer Architecture
3. [1697 2019] Root Mean Square Layer Normalization

### 1. [329 2019]  Transformers Without Tears - Improving the Normalization of Self-Attention

**Paper Link:** [Transformers Without Tears](https://arxiv.org/abs/2105.07576)

#### Problem Addressed
- Standard Layer Normalization (LN) has limitations in deep Transformers or long-sequence tasks.
- Training can be unstable due to gradient explosion or vanishing, especially with Post-LN placement.
- Sensitivity to hyperparameters (learning rate, initialization) makes optimization challenging.

#### Key Contributions
1. **Pre-LN vs Post-LN Analysis**: Detailed study of how normalization placement affects gradient flow, convergence speed, and stability.
2. **ScaleNorm and PowerNorm**: 
   - ScaleNorm: Normalizes by L2 norm and scales with a learned parameter, avoiding mean-centering.
   - PowerNorm: Similar idea but modifies the scaling to improve numerical stability.
3. **Residual Connection Adjustments**: Slight modifications in residual block placement to further enhance training stability.
4. **Empirical Validation**: Extensive experiments on NLP tasks such as language modeling, translation, and long-context tasks.

#### Detailed Takeaways
- Pre-LN improves gradient flow, allowing very deep Transformers to train without instability.
- ScaleNorm offers computational savings while maintaining or improving performance.
- Proper normalization reduces the occurrence of extreme attention values (outliers), leading to more stable optimization.
- The paper provides guidelines for training deep Transformers more efficiently and reliably, especially for long sequences or large models.
- Highlights the interplay between normalization and residual structure, offering insight for future architectural design.

#### Significance
- Facilitates training of deep and large-scale Transformers.
- Reduces sensitivity to hyperparameters.
- Provides an alternative to standard LN for long-context models or resource-constrained training.


### 2. [1800 2020] On Layer Normalization in the Transformer Architecture

**Paper Link:** [On Layer Normalization in Transformers](https://arxiv.org/abs/2002.04745)

#### Problem Addressed
- The effect of Layer Normalization (LN) placement on Transformer training dynamics is not fully understood.
- Standard Post-LN design can cause convergence problems in deep networks.
- Need for theoretical and empirical analysis of LN's role in residual blocks and attention layers.

#### Key Contributions
1. **Pre-LN vs Post-LN Analysis**: Shows Pre-LN improves gradient flow and training stability in deep Transformers.
2. **Theoretical Insights**:
   - LN stabilizes hidden state distributions.
   - Explains why Pre-LN reduces gradient vanishing/explosion compared to Post-LN.
3. **Practical Recommendations**:
   - Use Pre-LN for deep models (≥24 layers) to avoid instability.
   - Guidance on initialization and learning rate settings when using LN.

#### Detailed Takeaways
- Pre-LN allows each residual block to receive well-normalized inputs, improving stability and convergence.
- Post-LN may slow down learning and is more sensitive to hyperparameter tuning in very deep models.
- Empirical experiments confirm Pre-LN's advantage for deep networks without sacrificing performance in shallow networks.
- Provides a foundational understanding of normalization dynamics, informing subsequent improvements like ScaleNorm and RMSNorm.
- Highlights that normalization interacts closely with residual connections and attention mechanisms, affecting gradient propagation and model training.

#### Significance
- Justifies the widespread adoption of Pre-LN in modern Transformers (GPT, T5, etc.).
- Establishes a basis for designing more efficient normalization schemes in deep architectures.
- Enables stable training and better scaling of Transformer models.

### 3. [1697 2019] Root Mean Square Layer Normalization

**Many modern models use Pre‑LayerNorm, but RMSNorm is often used in large language model variants (e.g., some LLaMA/Qwen implementations).**

This paper proposes RMSNorm, a variant of LayerNorm that only rescales by the root mean square (RMS) instead of centering + scaling by standard deviation.

- RMSNorm removes the mean‑centering step, reducing computational cost.
- It still preserves scale invariance (important for stable gradients).
- Experiments across NLP and vision tasks show comparable performance to LayerNorm while being faster (speedups of ~7–64%).
