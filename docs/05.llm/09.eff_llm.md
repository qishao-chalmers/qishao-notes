...
- **Pruning**
  ...
  - **Contextual pruning** selects the sparse state of each layer.
    ...
    *PowerInfer* utilizes the sparsity of activation to dynamically predict the hotactivated neurons of the next layer and computes them on the GPU, whereas other cold-activated neurons are computed on the CPU.
