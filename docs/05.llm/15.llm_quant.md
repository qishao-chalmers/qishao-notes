...
3. [2053 2023] GPTQ: Accurate Post-Training Quantization for Generative Pre-trained Transformers
4. [226 2023] SpQR: A Sparse-Quantized Representation for Near-Lossless LLM Weight Compression
5. [1627 2024] AWQ: Activation-aware Weight Quantization for LLM Compression and Acceleration
6. [5 2025] Give Me BF16 or Give Me Death”? Accuracy-Performance Trade-Offs in LLM Quantization
7. [139] Understanding and Overcoming the Challenges of Efficient Transformer Quantization
8. [43] Integer or Floating Point? New Outlooks for Low-Bit Quantization on Large Language Models
9. [26] FP8-LM: Training FP8 Large Language Models :+1: :+1: :+1: from Microsoft <br> *This is discussed that first order in adam could be FP8, but second order in adam should be FP16. and the weight should be reserved a copy of FP32 full-precision or FP16 with tensor scaling..*
10. [139] FP8 Formats for Deep Learning
11. [41] With Shared Microexponents, A Little Shifting Goes a Long Way :+1: from Meta, Microsoft :+1:
12. [34] Stable and low-precision training for large-scale vision-language models :+1: <br>
   *mentioned in Deepseek paper mixed precision training section. Not read yet.*
13. [Y2025] ParetoQ: Scaling Laws in Extremely Low-bit LLM Quantization
14. [47] Microscaling Data Formats for Deep Learning
15. [Unread 2 Y2024] To FP8 and Back Again: Quantifying the Effects of Reducing Precision on LLM Training Stability
16. [145] Efficient 8-Bit Quantization of Transformer Neural Machine Language Translation Model
17. [Unread 27]Dynamic Memory Compression: Retrofitting LLMs for Accelerated Inference
18. [45] PB-LLM: Partially Binarized Large Language Models
19. [55] QLLM: Accurate and Efficient Low-Bitwidth Quantization for Large Language Models
20. [33] SpinQuant: LLM quantization with learned rotations
21. [35] FP8 versus INT8 for efficient deep learning inference
22. [Y2024]Integer Scale: A Free Lunch for Faster Fine-grained Quantization of LLMs
23. [434] Integer Quantization for Deep Learning Inference: Principles and Empirical Evaluation
24. [73] FP8 Quantization: The Power of the Exponent
25. [UnRead 121] Training High-Performance and Large-Scale Deep Neural Networks with Full 8-bit Integers
26. [381] I-BERT: Integer-only BERT Quantization
27. [637] Training Deep Neural Networks with 8-bit Floating Point Numbers
28. [904] LLM.int8(): 8-bit Matrix Multiplication for Transformers at Scale
29. [UnRead 2568] DoReFa-Net: Training Low Bitwidth Convolutional Neural Networks with Low Bitwidth Gradients :+1:
30. [239] Model Accuracy and Runtime Tradeoff in Distributed Deep Learning: A Systematic Study :+1:
31. [270] Hybrid 8-bit Floating Point (HFP8) Training and Inference for Deep Neural Networks <br> *This is the paper where E5M2 and E4M3*
32. [Read Y2025 NVIDIA] Coat: Compressing Optimizer States and Activation for Memory-Efficient FP8 Training  :+1:  :+1:  :+1:  :+1:  :+1: <br> *Source code: https://github.com/NVlabs/COAT*
33. [38 2023] Efficient Post-training Quantization with FP8 Formats
34. [8 2025 ByteDance] 70% Size, 100% Accuracy: Lossless LLM Compression for Efficient GPU Inference via Dynamic-Length Float
35. [5 2025] When Compression Meets Model Compression: Memory-Efficient Double Compression for Large Language Models
36. [Bytedance 2025] INT v.s. FP: A Comprehensive Study of Fine-Grained Low-bit Quantization Formats :+1: :+1:
37. [5 2025] FlexQuant: A Flexible and Efficient Dynamic Precision Switching Framework for LLM Quantization
...
> In BERT-like models, an intermediate hidden activation tensor x has a shape (B, T, d), where B is the batch size, T is the sequence length, and d is the number of embedding dimensions (d = 768 for BERT-base, Devlin et al. 2019).
...
---
## 3. [2053 2023] GPTQ: Accurate Post-Training Quantization for Generative Pre-trained Transformers
GPTQ process model layer by layer and using a small amout of calibration data to calculate Hessian Matrix.
(This Hessian Matrix is based on calcualting X*Transpose(X) X is the intermedidate vector.)

So Hessian Matrix show the imporance of weights, which means that whether the weight element will have huge imapct on output.
Also, it has eror-compsenate mechanism. It quantized the model layer by layer, it tuned later weight to get approximate result. This avoid the accumulation of error across layers.

---
## 4.[1627 2924] AWQ: Activation-aware Weight Quantization for LLM Compression and Acceleration

AWQ argues that GPTQ overfits to calibration data set.

AWQ recognize the outlier of hiddent state channel and scale corresponding channel into weights corresponding channel.

<img width="1005" height="228" alt="image" src="https://github.com/user-attachments/assets/aaee6527-d30a-4c7a-9ac1-785ea7a1dc2f" />

---
## 5. [226 2023] SpQR: A Sparse-Quantized Representation for Near-Lossless LLM Weight Compression

They found the outliers are not only in hidden states, but also in weights, structral weights.

<img width="709" height="351" alt="image" src="https://github.com/user-attachments/assets/ecfa56af-426a-4243-9f11-a6b1b9e4457f" />

In particular, they also found that deeper layer are more sensitive to weight quantization errors.
---
## 5. [5 2025] Give Me BF16 or Give Me Death”? Accuracy-Performance Trade-Offs in LLM Quantization

**This paper argues that GPTQ does not have the issue of overfitting, which claimed by AWQ**

1. W8A8-FP quantization is essentially lossless, preserving the uncompressed model’s accuracy across all benchmarks, often within the evaluation’s margin of error.
This result is achieved with a simple yet robust approach: dynamic per-token activation quantization combined with symmetric weight quantization via round-to-nearest assignment.
2. W8A8-INT quantization exhibits only amodest accuracy degradation of 1–3% per task on average, far lower than the 10%+drops reported in prior work (Li et al., 2024a;Lee et al., 2024b).
   This performance is enabled by dynamic activation quantization or SmoothQuant (Xiao et al., 2022), paired with GPTQ (Frantar et al., 2022) for symmetric weight quantization.
4. W4A16-INT quantization maintains consistently low accuracy loss, performing on par with W8A8-INT.
   Surprisingly, we show for the first time that a simple variant of GPTQ outperforms the more recent AWQ method (Lin et al., 2024a) on real-world tasks, challenging prior assumptions about low-bit quantization strategies.

> This finding contrasts with prior studies (Lin et al.,2024a; Huang et al., 2024), which favored AWQ or found it tied on academic subsets. We attribute this to three key factors:
> (1) we use GPTQ with MSEoptimal clipping (the AWQ comparison used absmax); this has no overhead and yields consistently better results;
> (2) we use higher-quality calibration data than the C4 default;
> (3) we include real-world benchmarks, providing a broader evaluation scope.
...
![image](https://github.com/user-attachments/assets/ca5c4914-cbf2-4cc2-8d80-eb423ef1aa10)
...
❗ It’s all about the outliers. If a distribution has very significant outliers, the FP8-E4/FP8-E5 format is more accurate.
...
## 28. [Read Y2025 NVIDIA] Coat: Compressing Optimizer States and Activation for Memory-Efficient FP8 Training  :+1:  :+1:  :+1:  :+1:  :+1:
...
![image](https://github.com/user-attachments/assets/cc6a4e9d-fbc3-4644-aaed-82b82a7adae3)
...
**the dynamic range for first-order momentum is typically less than 1e4, and for second-order momentum, it is
usually less than 1e1—both far below the available range of FP8. That is what they mean by second-order momentum is underutilized FP8.**

![image](https://github.com/user-attachments/assets/3fe1c480-39a1-4392-9af0-30fb7ab5ef36)
...
For non-linear layers, VS-Quant (Dai et al., 2021) or PerBlock Quant (Xi et al., 2024) methods are well-suited due to their fine-grained and precise nature.
...
![image](https://github.com/user-attachments/assets/9caf39d6-c382-4be3-8be6-9542c61dc703)
...
> FP8 formats outperform INT8 in multiple aspects, including workload coverage (92.64% vs. 65.87%), model accuracy and suitability for a broader range of operations.
> Furthermore, our findings suggest that E4M3 is better suited for NLP models, whereas E3M4 performs marginally better than E4M3 on computer vision tasks.

**Weight and Activation Scaling**
They recommend using per-channel scaling for weights across all networks.

**First and Last Operator**
The first convolution and the last fully-connecteds are more sensitive to quantization.

**Mixed FP8 Formats**

The **weight distributions** in both classes of models tend to follow **normal distributions** with lots values near zero.\n
These tensors require more mantissa bits in the data format to represent the distribution accurately.\n
In contrast, **activations of NLP models show a lot of outliers which demand a larger dynamic range** in the data format to ensure the outliers are accurately represented.\n
We balance this trade-off by assigning E5M2 or E4M3 format for range bound tensors and E3M4 for precision-bound tensors.

---
## 34. [8 2025 ByteDance] 70% Size, 100% Accuracy: Lossless LLM Compression for Efficient GPU Inference via Dynamic-Length Float 

In short, they use Dynamic-Length Float (DFloat11), based on the insight that the entropy of exponent of BF16 is very low. So they can use entropy-based compression format for storing exponent.

They also implement specific CUDA kernel for utilization of this feature.

<img width="680" height="248" alt="image" src="https://github.com/user-attachments/assets/8d6abc5f-8d90-4e99-8208-f682f4acb64d" />

<img width="794" height="344" alt="image" src="https://github.com/user-attachments/assets/616f8dc3-0c99-4a92-bd20-ce58310ad524" />

---

## 35. [5 2025] When Compression Meets Model Compression: Memory-Efficient Double Compression for Large Language Models

They found the activations are uneven distribution, thus results in higher compressibility.\n
And weights in NLP are even distributed, so they would like to scale the weights, so more bits could be quantized to zero and it also could be **uneven** distributed.

<img width="300" height="508" alt="image" src="https://github.com/user-attachments/assets/6ff4d333-e47d-408b-8163-1e952279911b" />


<img width="525" height="435" alt="image" src="https://github.com/user-attachments/assets/f6aa6b84-fe55-45b4-8df8-d3a5bfbe14a9" />

---

## 36. [Bytedance 2025] INT v.s. FP: A Comprehensive Study of Fine-Grained Low-bit Quantization Formats

> We reveal a critical performance crossover: while FP excels in coarse-grained quantization, the comparison at fine-grained (block-wise) levels is more nuanced.
...
> These findings challenge the current hardware trajectory, demonstrating that a one-size-fits-all FP approach is suboptimal and advocating that fine-grained INT formats, particularly MXINT8, offer a better balance of accuracy, power, and efficiency for
future AI accelerators

**Coarse-Grained (Large Blocks)**: FP formats (like FP8) are better because their wider dynamic range handles activation outliers across a large group of numbers.

**Fine-Grained (Small Blocks, e.g., size 32)**: INT formats (like INT8) become superior. When you break data into small blocks, the "outliers" are isolated. Within a small block, the data distribution is more uniform, and the linear/uniform distribution of INT represents this data more accurately than the non-uniform spacing of FP
...
### Key Findings
- Dynamic Precision Tolerance
  - LLMs can tolerate lower precision in weights as decoding progresses because prediction confidence increases.
  - Early tokens need higher precision; later tokens can use INT4 instead of INT8 with minimal accuracy loss (Section 3.2, Fig. 3).
- Layer-Wise Sensitivity
  - Not all layers react equally to low-bit quantization.
  - KL divergence analysis identifies which layers are safe to switch to lower precision first (Section 3.1, Fig. 2).
- Token-Wise Precision Scheduling
  - Using perplexity entropy (PPLE) over a sliding window of generated tokens allows real-time, fine-grained adjustment of precision during inference (Section 4.4, Fig. 5).
- Mixed-Precision Weight Storage
  - Storing both INT8 and INT4 copies of weights enables fast, dynamic switching without reloading weights from storage.
  - Prefill stage uses INT8 for speed; decode stage dynamically switches layers to INT4 for efficiency.
- Practical Performance Gains
  - End-to-end speedup of up to 1.3× for long-context generation tasks.
  - Maintains near INT8-level accuracy while enabling aggressive compression (Tables 1–2, Fig. 6–7).
  - Offers controllable trade-off between compression and quality via the precision-perplexity threshold.
### Key Takeaways
- Dynamic, token-level quantization is more effective than static quantization for LLMs, especially in long-context generation.
- Perplexity entropy (PPLE) is a practical metric to guide safe precision reduction.
- Layer-aware mixed-precision allows efficient use of hardware and memory bandwidth while minimizing accuracy loss.
- FlexQuant demonstrates that speed and memory efficiency can be significantly improved without retraining or complex post-training adjustments.
- Provides a flexible framework for deploying LLMs in resource-constrained or real-time scenarios.
